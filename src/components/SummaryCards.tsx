...
export default function SummaryCards({ totalCreditCard, effectiveSalary, effectiveFixed, monthMultiplier }: SummaryCardsProps) {
  ...
  const cards = [
    ...
    {
      label: netSavings >= 0 ? "Net Savings" : "Net Deficit",
      sub: netSavings >= 0 ? `${savingsRate.toFixed(1)}% of income saved` : `Overspending by ${(Math.abs(savingsRate)).toFixed(1)}%`,
      value: `${netSavings >= 0 ? "+" : "-"}${formatCurrency(Math.abs(netSavings))}`,
